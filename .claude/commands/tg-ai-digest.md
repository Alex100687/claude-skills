...
## Шаг 2б: Скачивание медиа для выбранных новостей ⚠️ ОБЯЗАТЕЛЬНЫЙ ШАГ — не пропускать

После отбора новостей — записать JSON с выбранными новостями и запустить `fetch_media.py`. Выполнить **всегда**, даже в автоматическом режиме.

**Slug** = название новости в lower_snake_case, например `gpt_5_4`, `runway_characters`, `photoshop_rotate`.

**Шаг 2б.1** — записать файл `My_Knowledge_Base/Digest_Media/media_selected.json` с выбранными новостями.
...
**Шаг 2б.2** — запустить скрипт (он прочитает JSON и скачает медиа):
...
## Шаг 3: Оформление в стиле @VAI_ART
...
## Шаг 4: Показать пост пользователю

Если запущен интерактивно:
1. Выведи готовый пост
2. Выведи **«Запасные новости»** — ещё до 10 новостей, которые прошли фильтр продуктовых новостей (Шаг 2), но не вошли в топ. Формат: нумерованный список, каждая — одна строка с кратким описанием (название продукта + что произошло). Это нужно чтобы пользователь мог заметить пропущенное и попросить заменить.
3. Спроси подтверждение или замены.

Если запущен в автоматическом режиме (`-p` флаг, headless) — сразу переходи к Шагу 5 без ожидания.
